...
function StickyNote({ note, onUpdate, onDelete, onDragStart, autoFocus }) {
  ...
  return (
    <div className="sn-note" style={{ left: note.x, top: note.y, background: colorObj.bg, transform: `rotate(${note.rotation}deg)`, zIndex: note.zIndex || 1 }} onMouseDown={(e) => onDragStart(e, note.id)}>
      <div className="sn-tape" style={tapeCss} />
      <textarea ref={taRef} className="sn-text" placeholder="Write something nice…" value={note.text} onChange={(e) => onUpdate(note.id, { text: e.target.value })} style={{ fontSize: sizeObj.fontSize, lineHeight: sizeObj.lineHeight }} onMouseDown={(e) => e.stopPropagation()} />
      <div className="sn-footer">
        <input className="sn-author" placeholder="— your name" value={note.author || ''} onChange={(e) => onUpdate(note.id, { author: e.target.value })} onMouseDown={(e) => e.stopPropagation()} />
...
function Sticker({ sticker, onDragStart, onDelete }) {
  return (<div className="sn-sticker" style={{ left: sticker.x, top: sticker.y, fontSize: sticker.size, zIndex: sticker.zIndex || 50 }} onMouseDown={(e) => onDragStart(e, sticker.id, 'sticker')} onDoubleClick={() => onDelete(sticker.id)} title="Drag · double-click to remove">{sticker.emoji}</div>);
}
...
export default function StickyNotesPage() {
  ...
  useEffect(() => {
    function onMove(e) {
      ...
      if (dragging.current.kind === 'sticker') setStickers(prev => prev.map(s => s.id === dragging.current.id ? { ...s, x: dragging.current.origX + dx, y: dragging.current.origY + dy } : s));
      else setNotes(prev => prev.map(n => n.id === dragging.current.id ? { ...n, x: dragging.current.origX + dx, y: dragging.current.origY + dy } : n));
    }
  ...
  function handleBoardClick(e) {
    ...
    if (mode === 'note') {
      const id = Date.now();
      setNotes(prev => [...prev, { id, x: x - 95, y: y - 90, text: '', author: '', color: selectedColor, tape: selectedTape, textSize: selectedSize, rotation: (Math.random() - 0.5) * 6, zIndex: topZ.current }]);
      setJustCreated(id);
  ...
  function handleReset() {
    if (window.confirm('Clear all notes and restore the original board?')) {
      ...
      setNotes(SEED_NOTES); setStickers(SEED_STICKERS);
    }
  }
    ...
    <div className="fj-page">
      ...
      <div className="fj-board" ref={boardRef} style={{ cursor: boardCursor }} onClick={handleBoardClick}>
        <div className="fj-canvas">
          <div className="fj-canvas-text" style={{ left: 80, top: 160 }}>Leave me<br />a note</div>
          <div className="fj-canvas-hint" style={{ left: 88, top: 420 }}>Pick a color below → click the board → type your message ✦</div>
          {notes.map(n => (<StickyNote key={n.id} note={n} onUpdate={handleUpdateNote} onDelete={handleDeleteNote} onDragStart={handleDragStart} autoFocus={n.id === justCreated} />))}
          {stickers.map(s => (<Sticker key={s.id} sticker={s} onDragStart={handleDragStart} onDelete={handleDeleteSticker} />))}
        </div>
      </div>
      {showStickers && (
        <div className="fj-sticker-tray">
          {STICKERS.map(em => (<button key={em} className={`fj-sticker-btn${activeSticker === em && mode === 'sticker' ? ' active' : ''}`} onClick={() => { setActiveSticker(em); setMode('sticker'); setShowStickers(false); }}>{em}</button>))}
        </div>
      )}
      <div className="fj-toolbar">
        ...
        <div className="tb-section"><span className="tb-label">Color</span><div className="tb-row">{NOTE_COLORS.map(c => (<button key={c.id} className={`tb-color${selectedColor === c.id ? ' active' : ''}`} style={{ background: c.bg, border: c.id === 'white' ? '1px solid rgba(255,255,255,0.25)' : undefined }} onClick={() => setSelectedColor(c.id)} title={c.label} />))}</div></div>
        <div className="tb-sep" />
        <div className="tb-section"><span className="tb-label">Tape</span><div className="tb-row">{TAPE_STYLES.map(t => (<button key={t.id} className={`tb-tape${selectedTape === t.id ? ' active' : ''}`} style={{ ...t.css }} onClick={() => setSelectedTape(t.id)} title={t.label} />))}</div></div>
        <div className="tb-sep" />
        <div className="tb-section"><span className="tb-label">Size</span><div className="tb-row">{TEXT_SIZES.map(s => (<button key={s.id} className={`tb-size${selectedSize === s.id ? ' active' : ''}`} onClick={() => setSelectedSize(s.id)}>{s.label}</button>))}</div></div>
        <div className="tb-sep" />
        <button className={`tb-post${mode === 'note' ? ' armed' : ''}`} onClick={() => { setMode(m => m === 'note' ? 'select' : 'note'); setShowStickers(false); }}>
